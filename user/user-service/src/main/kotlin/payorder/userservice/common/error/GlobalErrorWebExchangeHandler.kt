...
class GlobalErrorWebExchangeHandler(
    ...
    private fun handleError(request: ServerRequest): Mono<ServerResponse> =
        when(val throwable = super.getError(request)) {
            is UserBasicException -> buildErrorResponse(throwable)
            is ConnectException -> buildErrorResponse(UserBasicException("Cannot Connect to Service", HttpStatus.INTERNAL_SERVER_ERROR))
            else -> buildErrorResponse(UserBasicException("Internal Server Error", HttpStatus.INTERNAL_SERVER_ERROR))
        }

    private fun buildErrorResponse(ex: UserBasicException) =
